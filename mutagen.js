function mutateCode(code, mutation_chance) {
	// console.log("mutateCode with mutation_chance=", mutation_chance);
	// TODO: remove \[ when adding compile-fail checking
	// TODO: replace negative_match_group with negative lookahead if possible
	var new_code = code.replace(/([^a-z$_\[]|^)(0x[\dA-F]+|\d+(?:\.\d*)?|\.\d+)(?!e)/gi, (match_str, negative_match_group_str, num_str)=> {
		// console.log(`matched ${num_str} (full match_str = ${match_str})`);
		if (isNaN(num_str)) { console.log(`somehow matched ${num_str}`); }
		var n = parseFloat(num_str);
		var original_n = n;
		// important: keep it as float or int (determined by whether there's a decimal point in GLSL)
		...
		if (Math.random() < mutation_chance) { n *= 2; }
		if (isNaN(n)) {
			console.log(`somehow got ${n} from ${JSON.stringify(num_str)}`);
			return negative_match_group_str + num_str;
		} else if (n < 0 && original_n >= 0) {
			return negative_match_group_str + "(" + cast(n - 1) + ")";
		}
		return negative_match_group_str + cast(n);
	});
	return new_code;
}
...
function findCompileFailure() {
	// TODO: bytebeat
	return document.querySelector(".tab.errorYes, .CodeMirror .errorMessage");
}
	...
	return new Promise((resolve, reject)=> {
		function waitForCompileFinish() {
			if (isCompiling()) {
				...
				var success = !findCompileFailure();
...
function mutateCodeOnPage() {
	...
	var new_code = mutateCode(original_code, mutation_chance);
